 ...
 */
public class WIPSimpleTransformer extends Transformer {
				...
				for (CampoSalida currentFila : currentTable.getCamposDeSalida()) {
					...
					for (CampoSalida currentColumna : currentFila.getCamposDeSalida()) {
						...
						if (!currentColumna.tieneHijos()) {
							...
							//Es una lista, campo complejo, o algo mas compuesto que un simple valor.
							String tableName=currentColumna.getTableName();
							getWriter().addTabla(tableName);
							try {
							
								Object resultado = solver.resolveMethodInvocation(currentColumna.getTarget(), instance);
								if (resultado instanceof List) {
										
									List listaDeElementos=(List) resultado;
									for (Object object : listaDeElementos) {
										this.getWriter().addFila();
										for (CampoSalida columnaActual : currentColumna.getCamposDeSalida()) {
											//Para cada elemento encontrado agrego una columna...
											resultado = solver.resolveMethodInvocation(columnaActual.getTarget(), object);
											if (resultado!=null) {
												this.getWriter().addColumna(columnaActual.getValue(), resultado);
											}
										}
									}
//									try {
//										getWriter().cerrarTabla();
//									} catch (IOException e) {
//										// TODO Auto-generated catch block
//										e.printStackTrace();
//									}
								}else{
									//Objeto compuesto simple, no coleccion.
								}
