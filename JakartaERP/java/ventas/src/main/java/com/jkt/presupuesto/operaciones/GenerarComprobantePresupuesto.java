...
public class GenerarComprobantePresupuesto extends Operation {
	...
	public void execute(Map<String, Object> aParams) throws Exception {
		...
		if (rutaCompartida.endsWith(LAST_CHARACTER)) {
			...
			/*
			 * Metodo principal
			 * A partir de un comprobante, se genera un reporte en PDF y retorna la ruta, que deberia ser compartida, en una instancia de la clase Especificaci�n
			 * 
			 */
	 ...
	 * no contiene elementos o la cantidad de elementos que retornan informaci�n son mas de uno.
	 */
...
//		parameters.put("firma", rutaCompartida.concat(RUTA_IMAGENES).concat(datosEmpresa.getFirma()));
	...
	private void validarDatosEmpresa() throws JakartaException {
		...
		if (datosEmpresa.getDireccion().isEmpty()) {
			throw new JakartaException("Debe ingresar la direcci�n de la empresa");
		}
	...
	private List<ItemResumen> obtenerDetalles() throws JakartaException {
		...
		for (PresupuestoDet detalle : detalles) {
			...
			case PresupuestoDet.CHAR_ELECTRICO:
				descripcion=String.format("Determinaci�n Laboratorio El�ctrico - Cantidad: %d - Determinacion: %s", cantidad ,detalle.getDeterminacion().getDescripcion());
				break;
			case PresupuestoDet.CHAR_QUIMICO:
				descripcion=String.format("Determinaci�n Laboratorio Quimico - Cantidad: %d - Determinacion: %s", cantidad ,detalle.getDeterminacion().getDescripcion());
				break;
			...
			case PresupuestoDet.CHAR_ITEM:
				...
				break;
			default:
				throw new JakartaException("No se generar� el comprobante debido a que los detalles del presupuesto son inconsistentes en cuanto a su tipo.");
			}
