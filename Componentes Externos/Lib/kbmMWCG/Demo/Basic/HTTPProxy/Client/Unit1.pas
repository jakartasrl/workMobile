...
uses
  ...
  kbmMemCSVStreamFormat, DBCtrls,
  kbmMWStreamFormat, ExtCtrls, ComCtrls,
  kbmMemBinaryStreamFormat,kbmMWHTTPStdTransStream, kbmMWBinaryStreamFormat,
...
type
  TForm1 = class(TForm)
    ...
    procedure qClientSideResolveError(Sender: TObject; ErrorType: Integer;
      ...
      var Retry: Boolean);
    procedure kbmMWTCPIPIndyClientTransport1Exception(Sender: TObject;
...
uses kbmMWGlobal,kbmMWExceptions;
...
begin
     ...
     kbmMWTCPIPIndyClientTransport1.ConnectionString.Assign(mConnection.Lines);
     kbmMWTCPIPIndyClientTransport1.Params.Assign(mParams.Lines);
...
begin
     ...
     try
        ...
        if kbmMWSimpleClient1.Cancelled then
           lbDebug.Items.Add('Cancelled')
        else
        begin
             slst.CommaText:=v;
             lbDebug.Items.AddStrings(slst);
...
procedure TForm1.kbmMWTCPIPIndyClientTransport1Exception(Sender: TObject;
  OldState: TkbmMWRequestStates; var NewState: TkbmMWRequestStates;
  E: Exception);
var
   EK:EkbmMWException;
begin
     if E is EkbmMWException then
     begin
          EK:=EkbmMWException(E);
          ShowMessage('Got EkbmMWException: '+Format('%s (%.8x)',[EK.Message,EK.ExceptionCode]));
     end
     else
         ShowMessage('Got Exception: '+E.Message);
     NewState:=rsCancel;
end;
